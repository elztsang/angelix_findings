...
//	if(data + sizeof(struct eth*) < data_end) printf("within bounds\n");
		if (ANGELIX_CHOOSE(bool, 1, 50, 3, 50, 11, ((char*[]){"argv", "data_end", "data", "x", "struct_size", "argc", "which"}), ((int[]){argv, data_end, data, x, struct_size, argc, which}), 7)) x = hdr.num2;	//good data
